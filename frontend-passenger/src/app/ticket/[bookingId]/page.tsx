...
export default function TicketPage() {
  ...
  const handleResend = async () => {
    setResending(true);
    try {
      await ticketsApi.resend(bookingId);
      toast.success('Ticket resent to your phone and email');
    } catch {
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/tickets/${bookingId}/pdf`, {
      ...
      a.href = url;
      a.download = `ticket-${ticket?.ticket_number ?? bookingId}.pdf`;
      a.click();
      ...
      <div className="px-4 py-5 space-y-4">
        ...
        <div className="bg-gray-900 rounded-2xl border border-gray-800 overflow-hidden">
          ...
          <div className="px-5 py-5 flex flex-col items-center gap-3">
            ...
            {ticket.qr_code_data ? (
              <Image src={ticket.qr_code_data} alt="QR Code" width={160} height={160} className="rounded-xl" unoptimized />
            ) : (
        ...
        <div className="flex gap-3">
          ...
          <button type="button" onClick={handleDownloadPDF} className="btn-secondary flex-1 flex items-center justify-center gap-2 py-3.5">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
