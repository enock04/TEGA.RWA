...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      (p) => { setCoords({ lat: p.coords.latitude, lon: p.coords.longitude }); setLocLabel(t('home.yourLocation')); },
      () => { setCoords(KIGALI); setLocLabel(t('home.defaultLocation')); },
      { timeout: 8000 }
    );
  }, []);
  ...
  if (isAuthenticated) {
    ...
    return (
      <MainLayout>
        <AppHeader title="TEGA.Rw" right={<LanguageSwitcher />} />

        {/* Greeting */}
        <div className="px-5 pt-4 pb-3">
          <p className="text-gray-400 text-sm">{t('home.goodToSeeYou')}</p>
        ...
        <div className="px-4 pt-5 space-y-3">
          <p className="text-gray-500 text-xs font-semibold uppercase tracking-wide">{t('home.quickActions')}</p>
          ...
          <Link href="/search"
            ...
            <div className="flex-1">
              <p className="text-white font-semibold text-sm">{t('home.bookABus')}</p>
              <p className="text-gray-500 text-xs">{t('home.bookABusDesc')}</p>
            </div>
          ...
          <Link href="/dashboard"
            ...
            <div className="flex-1">
              <p className="text-white font-semibold text-sm">{t('home.myTrips')}</p>
              <p className="text-gray-500 text-xs">{t('home.myTripsDesc')}</p>
            </div>
            <svg className="w-4 h-4 text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
            </svg>
        ...
        <div className="px-4 pt-6 pb-6">
          <div className="flex items-center justify-between mb-3">
            <p className="text-gray-500 text-xs font-semibold uppercase tracking-wide">{t('home.popularRoutes')}</p>
            <Link href="/search" className="text-xs text-gray-400 underline underline-offset-2">{t('common.seeAll')}</Link>
          </div>
          ...
          {routesLoading ? (
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 text-center">
              <p className="text-gray-500 text-sm">{t('home.noRoutes')}</p>
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {route.available_seats != null && (
                      <p className="text-gray-600 text-xs">{t('common.seatsLeft', { count: route.available_seats })}</p>
                    )}
                  </div>
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <p className="text-gray-600 text-xs">{t('common.perSeat')}</p>
                  </div>
                </Link>
              ))}
            </div>
    ...
    <MainLayout showNav={false}>
      <div className="flex flex-col min-h-screen bg-gray-950">
        <div className="flex-1 flex flex-col items-center justify-center px-8 text-center">
          ...
          <p className="text-gray-300 text-base font-medium mb-1">{t('home.tagline')}</p>
