...
export default function AdminBusesPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error((err as any).response?.data?.message || 'Failed to save bus');
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5 mb-6 shadow-card">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input className="input-admin" placeholder="e.g. Virunga Express 01" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
            </div>
            <div>
              <label className="label-admin">Plate Number *</label>
              <input className="input-admin" placeholder="e.g. RAB 001 A" value={form.plateNumber} onChange={e => setForm(f => ({ ...f, plateNumber: e.target.value }))} />
            </div>
            <div>
              <label className="label-admin">Bus Type</label>
              <select title="Bus type" className="input-admin" value={form.busType} onChange={e => setForm(f => ({ ...f, busType: e.target.value as BusType }))}>
                {(['standard', 'luxury', 'minibus', 'coach'] as BusType[]).map(t => <option key={t} value={t} className="capitalize">{t}</option>)}
              </select>
            </div>
            <div>
              <label className="label-admin">Total Seats *</label>
              <input type="number" className="input-admin" placeholder="e.g. 44" min="1" max="100" value={form.totalSeats} onChange={e => setForm(f => ({ ...f, totalSeats: e.target.value }))} />
            </div>
          </div>
          <div className="flex gap-3 mt-4">
            <button type="button" onClick={handleSave} disabled={saving} className="btn-admin-primary">{saving ? <Spinner size="sm" /> : null} {editing ? 'Update Bus' : 'Save Bus'}</button>
