    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Passenger bookings on your schedules</p>
        </div>
        <span className="text-sm text-gray-500 bg-white border border-gray-200 px-3 py-1 rounded-lg">{total} total</span>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-4 mb-4 flex flex-wrap gap-4">
        <div>
          ...
          <div className="relative flex items-center">
            <select className="input-admin w-40 pr-8" title="Filter by status" value={filters.status}
            ...
            {filters.status && (
              <button type="button" onClick={() => { setFilters(f => ({ ...f, status: '' })); setPage(1); }}
                className="absolute right-2 text-gray-400 hover:text-gray-600" aria-label="Clear">×</button>
            )}
          </div>
        </div>
        <div>
          ...
          <div className="relative flex items-center">
            <input type="date" className="input-admin w-44 pr-8" value={filters.date} title="Filter by date"
              onChange={e => { setFilters(f => ({ ...f, date: e.target.value })); setPage(1); }} />
            {filters.date && (
              <button type="button" onClick={() => { setFilters(f => ({ ...f, date: '' })); setPage(1); }}
                className="absolute right-2 text-gray-400 hover:text-gray-600" aria-label="Clear">×</button>
            )}
          </div>
        </div>
