...
export default function AgencySettingsPage() {
  ...
  const initials = user.full_name?.split(' ').map((n: string) => n[0]).join('').slice(0, 2).toUpperCase() ?? 'A';
    ...
    <div className="max-w-5xl space-y-6">
      <div className="flex items-start justify-between gap-6">
        ...
        <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl px-5 py-4 flex items-center gap-4 text-white flex-shrink-0">
          <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center text-white text-base font-bold flex-shrink-0 shadow-lg">
            {initials}
          </div>
          ...
          <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full border capitalize flex-shrink-0 bg-emerald-100 text-emerald-700 border-emerald-200">
            {user.role}
          </span>
        </div>
      </div>
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input className="input-admin" title="Full Name" placeholder="Full Name"
                    value={profileForm.fullName} onChange={e => setProfileForm(f => ({ ...f, fullName: e.target.value }))} />
                </div>
                <div>
              ...
              <div>
                ...
                <div className="relative">
                  <input className="input-admin opacity-60 cursor-not-allowed pr-24" title="Phone Number" value={user.phone_number} disabled />
              ...
              <button type="button" onClick={handleSaveProfile} disabled={savingProfile}
                ...
                {savingProfile ? <Spinner size="sm" /> : <SvgIcon d="M5 13l4 4L19 7" />}
                Save Changes
              </button>
            </div>
          ) : (
            <div className="space-y-5">
              ...
              {([
                ...
                <div key={field.key}>
                  ...
                  <div className="relative">
                    <input className="input-admin pr-10" type={field.show ? 'text' : 'password'} title={field.label}
                      placeholder={field.hint ?? '••••••••'} value={pwForm[field.key]}
              ...
              <button type="button" onClick={handleChangePassword} disabled={savingPw}
                className="btn-admin-primary flex items-center gap-2">
                {savingPw ? <Spinner size="sm" /> : <SvgIcon d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />}
                Update Password
              </button>
            </div>
          )}
