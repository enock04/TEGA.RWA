...
export default function AgencySchedulesPage() {
  ...
  const [editing, setEditing] = useState<AgencySchedule | null>(null);
  ...
  const openEdit = (s: AgencySchedule) => {
    setEditing(s);
  ...
  const handleSave = async () => {
    ...
    if (new Date(form.arrivalTime) <= new Date(form.departureTime)) {
      toast.error('Arrival time must be after departure time'); return;
    }
    setSaving(true);
    try {
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Manage your departure schedules</p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5 mb-6 shadow-card">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="datetime-local" title="Departure time" className="input-admin" value={form.departureTime}
                onChange={e => setForm(f => ({ ...f, departureTime: e.target.value }))} />
            </div>
            <div>
              <label className="label-admin">Arrival Time *</label>
              <input type="datetime-local" title="Arrival time" className="input-admin" value={form.arrivalTime}
            ...
            <div>
              ...
              <input type="number" title="Price" className="input-admin" placeholder="e.g. 3500" value={form.basePrice}
      ...
      {loading ? (
        ...
        <EmptyState title="No schedules yet" description="Create your first schedule to start accepting bookings." />
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  {['Route', 'Bus', 'Departure', 'Arrival', 'Price', 'Seats', 'Status', ''].map(h => (
              ...
              <tbody className="divide-y divide-gray-50">
                {schedules.map(s => (
                  <tr key={s.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 font-semibold text-green-700">RWF {Number(s.base_price).toLocaleString()}</td>
                    ...
                    <td className="px-4 py-3 text-right flex items-center justify-end gap-3">
                      {s.status === 'active' && (
                        <>
                          ...
                          <button type="button" onClick={() => handleCancel(s.id)} className="text-xs text-red-500 hover:text-red-700 font-medium">Cancel</button>
                        </>
                      )}
                    </td>
                  </tr>
